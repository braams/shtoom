...
{
    return [self initWithCapacityFrames:frames channels:channels interleaved:NO];
}

- initWithCapacityFrames:(unsigned)frames channels:(unsigned)channels interleaved:(BOOL)interleaved
{
	if ( 0 == channels )
	{
		[self dealloc];
	...
	myBuffer = MTAudioBufferListNew ( channels, frames, interleaved );
	if ( NULL == myBuffer )
	{
		[self dealloc];
		return nil;
